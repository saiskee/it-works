...
/* GET /
  * Will Return all the questions previously created by the manager
 */
questionRoutes.get('/', async (req, res) => {
  const {userId} = req.session.user;
  const questions = await Question.find({author: mongoose.Types.ObjectId(userId)});
  const questions_res = questions.map(question => {
    return {_id: question._id, question_data: question.question_data, name: question.name, author: question.author};
  });
